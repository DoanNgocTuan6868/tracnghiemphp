<?php 
  class cauhoi{
      ...
      public function insertCauhoi($noidung,$dapanA,$dapanB,$dapanC,$dapanD,$ketqua,$made)
      {
        ...
        $db = new connect(); // tạo 1 đối tượng connect
        $stm = $db->execP($query);
      ...
      public function Question($made){
          ...
          b.soluongcauhoi,b.thoigian from cauhoi a, de b, mon c where a.maDe=b.maDe and b.maMon=c.maMon and a.maDe=$made";
          
          $result=$db->selectone($select);
          return $result;
      }
      // truy vấn lấy danh sách câu hỏi theo mã đề , và sắp xếp câu hỏi theo thư tự ngẫu nhiên
    ...
    function getListCauhoi($maND)
    {
        ...
        $select = "select * from cauhoi where maND=$maND"; // lấy tất cả các trường tt của câu hỏi có mand = với mnd đc truyền vào hàm
        $db = new connect();
        $result = $db->selectone($select);// gọi selcetone để trả  về kq , nếu nhiều hơn một câu hỏi có mand thì hàm trả về thông tin câu hỏi đầu tiên
        return $result;
    }
